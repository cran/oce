...
<<label=ctdfig, width=6, height=5, fig=TRUE, echo=FALSE>>=
<<ctdplot>>
@
\end{center}
...
<<label=ctdpycnoclineplot, include=FALSE>>=
library(oce)
data(ctd)
...
<<label=ctdpycnoclinefig, fig=TRUE, width=6, height=4, echo=FALSE>>=
<<ctdpycnoclineplot>>
@
\end{center}
...
\begin{center}
\setkeys{Gin}{width=\textwidth}
<<label=ctdrawfig,fig=TRUE,width=8, height=4, echo=FALSE>>=
<<ctdrawplot>>
@
\end{center}
...
\begin{center}
\setkeys{Gin}{width=\textwidth}
<<label=sealevelfig,fig=TRUE,width=10,height=13,echo=FALSE>>=
<<sealevelplot>>
@
\end{center}
...
<<label=lobo2,fig=FALSE>>=
a <- as.numeric(lobo$time - lobo$time[1])
hue <- 0.5 * a / max(a)
plot.TS(as.CTD(lobo$S, lobo$T, 0), col.data=hsv(hue,1,1), pch=20,cex=2)
@
